local lsp = require('lsp-zero')

lsp.preset('recommended')

local cmp = require('cmp')
local cmp_select = {behavior = cmp.SelectBehavior.Select}
local cmp_mappings = lsp.defaults.cmp_mappings({
  ['<C-p>'] = cmp.mapping.select_prev_item(cmp_select),
  ['<C-n>'] = cmp.mapping.select_next_item(cmp_select),
  ['<C-y>'] = cmp.mapping.confirm({ select = true }),
  ["<C-Space>"] = cmp.mapping.complete(),
})

cmp_mappings['<Tab>'] = nil
cmp_mappings['<S-Tab>'] = nil

lsp.setup_nvim_cmp({
  mapping = cmp_mappings
})

lsp.set_preferences({
    suggest_lsp_servers = false,
    sign_icons = {
        error = 'E',
        warn = 'W',
        hint = 'H',
        info = 'I'
    }
})

lsp.on_attach(function(client, bufnr)
  local opts = {buffer = bufnr, remap = false}
  local bind = vim.keymap.set

  bind('n', 'gd', '<cmd> lua vim.lsp.buf.definition()<cr>', opts)
  bind("n", "K", '<cmd> lua vim.lsp.buf.hover()<cr>', opts)
  bind("n", "<leader>wss", '<cmd>lua vim.lsp.buf.workspace_symbol()<cr>', opts)
  bind("n", "<leader>cd", '<cmd>lua vim.diagnostic.open_float()<cr>', opts)
  bind("n", "[d", '<cmd>lua vim.diagnostic.goto_next()<cr>', opts)
  bind("n", "]d", '<cmd>lua vim.diagnostic.goto_prev()<cr>', opts)
  bind("n", "<leader>ca", '<cmd>lua vim.lsp.buf.code_action()<cr>', opts)
  bind("n", "<leader>cD", '<cmd>lua vim.lsp.buf.references()<cr>', opts)
  bind("n", "<leader>cr", '<cmd>lua vim.lsp.buf.rename()<cr>', opts)
  bind("i", "<C-h>", '<cmd>lua vim.lsp.buf.signature_help()<cr>', opts)
end)


lsp.setup()

vim.diagnostic.config({
    virtual_text = true
})

